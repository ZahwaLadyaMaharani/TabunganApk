...
public class DatabaseAcces {
    ...
    public void update(CatatanTabungan catatanTabungan){
        ContentValues values = new ContentValues();
        values.put("date", new Date().getTime());
        values.put("catatan pemasukan dan pengeluaran", catatanTabungan.getText());
        String date = Long.toString(catatanTabungan.getTime());
        database.update(DatabaseOpenHelper.TABLE,values, "date = ?", new String[]{date});

    }

    public void delete(CatatanTabungan catatanTabungan){
        String date = Long.toString(catatanTabungan.getTime());
    ...
    public List getAllMemos(){
        ...
        while (!cursor.isAfterLast()){
            ...
            memos.add(new CatatanTabungan(time, text));
            cursor.moveToNext();
        }
